//
// Created by Damian Netter on 13/05/2025.
//

#pragma once

#include "ZNBKit/jni/internal/wrapper.hpp"
#include "ZNBKit/jni/signatures/method_signature.hpp"

namespace znb_kit
{
    class int_method final : public method_signature<jint>
    {
    public:
        using method_signature::method_signature;

        jint invoke(const jobject &instance, std::vector<local_value_reference> &parameters) override
        {
            return wrapper::invoke_int_method(env, get_owner(), instance, get_identity(), parameters);
        }
    };
}
